...
class EdgePredictor(nn.Module):
    ...
    def __init__(self, embedding_dim, device):
        ...
        #self.individual_query = ISABStack(1, embedding_dim, 256, num_heads = 4, 
        #                                  num_inds = 16, device = device)
    ...
    def forward(self, E, V):
        ...
        #G = torch.sum(Z1 * Z1, dim = 2).unsqueeze(2)
        #H = torch.sum(Z1 * Z1, dim = 2).unsqueeze(1)
        ...
        return  F * torch.exp(self.scale1) + self.baseline #+ \
         ...
         #      self.baseline
        #return torch.exp(self.scale1) * (G + H - 2 * F) + self.baseline
